...
Servo myservo; //myservo-->servo object
...
void setup() {
  ...
  pinMode(ldr2, INPUT); //set W `est ldr pin as an input
...
void loop() {
  ...
  if (pos > 130) {
    pos = 130; // reset the horizontal postion of the motor to 130 if it tries to move past this point
  }
  if (pos <60) {
    pos = 60; // reset the horizontal position of the motor to 60 if it tries to move past this point
  }
